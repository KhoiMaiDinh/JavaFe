...
import { usePostPinMutation } from '../features/pin/pinApiSlice';
...
const CreatePin = () => {
  ...
  // const [postPin, {isLoading}] = usePostPin();
  
  // useEffect(() => {
  //   setErrMsg("");
  // }, [user, pwd]);

  const [postPin] = usePostPinMutation();
  ...
  const savePin = async () => { try{
    ...
    if(title && imageAsset && category) {
      ...
      // const result = await postPin({formData}).unwrap();
      // console.log({result})
      try{
        const response = await axios.post('http://localhost:8080/api/v1/pin', formData, {
          headers:{
