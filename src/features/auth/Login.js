...
const Logins = () => {
  ...
  const [login, getUserInfo, { isLoading }] = useLoginMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = await login({ user, pwd }).unwrap()
      .then(token => dispatch(setCredentials({ token: token.access_token, user: null})))
      .then(() => {
        getUserInfo().unwrap();
      })
